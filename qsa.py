...
def get_QSA_JSON(cnpj):
    ...
    for table in QSAconn.tables:
        QSAconn.cursor.execute("SELECT * FROM {} WHERE cnpj='{}'".format(table, cnpj))
        ...
        if(queryResult != None):
            return {
                "qsa": {
                    ...
                    "situacao": convertToNum(queryResult[EnumQSA.situacao]),
                    "data_situacao": queryResult[EnumQSA.data_situacao],
                    "motivo_situacao": convertToNum(queryResult[EnumQSA.motivo_situacao]),
